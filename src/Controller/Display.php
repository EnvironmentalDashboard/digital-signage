        ...
        foreach ($display->getPresentations() as $presentation) {
            ...
            foreach ($map as $relation) {
                ...
                foreach ($carousel->getFrames() as $i => $frame) {
                    ...
                    $iframes[] = "<iframe sandbox='allow-scripts allow-same-origin allow-pointer-lock' src='{$frame->getUrl()}' id='frame{$frame->getId()}' data-duration='{$frame->getDuration()}' frameborder='0' style='width: 100%;height: 100%;{$hidden}'></iframe>";
                }
