...
then
	# prod env:
	docker run -dit -p 5000:80 -p 5001:8080 --restart always -v $(pwd)/var:/var/www/html/var/ --name PROD_DS digital-signage
else
	# dev env:
	# (bind mounting code so changes to code don't require image rebuild)
	docker run -dit -p 5000:80 -p 5001:8080 --restart always -v $(pwd)/var:/var/www/html/var/ -v $(pwd):/var/www/html/ -e ENV=DEV --name DEV_DS digital-signage
fi
