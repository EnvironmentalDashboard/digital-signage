...
then
	# prod env:
	docker run -dit -p 5000:80 --restart always --name PROD_DS digital-signage
else
	# dev env:
	# (bind mounting code so changes to code don't require image rebuild)
	docker run -dit -p 5000:80 --restart always -v $(pwd):/var/www/html/ --name DEV_DS digital-signage
fi
